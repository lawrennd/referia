...
The library builds on functionality provided in [`ndlpy`](https://github.com/lawrennd/ndlpy/). The main difference between the two is that functionality that is general for the flow-based model the code follows sits in `ndlpy`. The `referia` code is the place where functionality that is specific to interacting with the data (such as reviewing) is provided. 
