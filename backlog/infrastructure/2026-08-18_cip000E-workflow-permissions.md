...
- [ ] Both workflow files declare `permissions:` at workflow or job level
- [ ] `python-tests.yml` and `docs.yml` `test-coverage` are read-only for contents
- [ ] Docs deploy job still has enough token scope to publish GitHub Pages
- [ ] CodeQL alerts #1–#3 close after the change reaches `main` (or are expected to close on next scan)

## Implementation Notes

Verify against current `peaceiris/actions-gh-pages@v3` docs before adding write scopes. Do not
grant `contents: write` unless the action actually needs it.
